...
app.use('/img', express.static(path.join(__dirname, 'uploads'))); // /img 경로로 uploads 내 파일 가져오기 / 
...
app.use(passport.initialize()); //req 객체에 passport 설정을 심음(req. user, req.login, req.isAuthenticate, req.logout) 
app.use(passport.session()); //connect.sid 라는 이름으로 세션 쿠기가 브라우저에 전송, req.session 객체에 passport 설정 저장. 
